...
class SaleReportController extends AppController {
    ...
    public function getCheckoutDetail() {
	   $this->layout = '';
	   $query = "select * from checkouts inner join users on users.id = checkouts.user_id inner join payment_states on payment_states.id = checkouts.payment_state_id inner join payments_methods on payments_methods.id = checkouts.payment_method_id where checkouts.id = 75;";
